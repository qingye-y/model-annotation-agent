# 任务调度 & 标注任务模块 PRD v3.0

**版本**：v3.0（任务调度 & 标注任务模块专项）
**日期**：2026-05-25
**产品经理**：李子玥（青也）
**审查协助**：小B
**文档状态**：持续完善中
...
| v3.0 | 2026-05-25 | 任务调度 & 标注任务模块 v3.0：批次号统一（DISP-YYYYMMDD-NNN）、分配时间时区修复（UTC→北京时间）、撤回与删除功能（状态流转）、批量撤回（复选框）、标注任务页返回按钮+侧边栏联动、统计摘要改为3个准确率进度条、列表增加合规/违规数 | 小B |
| v3.1 | 2026-05-26 | 机审中心与调度中心全链路联动 v3.1：生成标注任务双按钮（重新生成+查看生成任务）、明细页任务分配状态筛选与展示、调度中心 filter_batch URL 筛选、重新生成智能撤回逻辑（只撤回未领取任务）、RawData.task_status 状态字段 | 小B |

---

## 附：v3.0 功能变更总览

> 以下为本 v3.0 相较 v2.x 的全部功能变更，适用于**任务调度中心**和**标注任务页**两个模块。

### 附-1 分配时间时区修复

**问题**：`created_at` 以 UTC 存储，前端直接显示导致时间少 8 小时。

**方案**：
- 后端 `fmt_bj(dt)` 函数：`UTC datetime → 北京时间（UTC+8）` 格式化
- `bj_today_utc()` 函数：计算北京时间今日 00:00 对应的 UTC datetime
- `_today_assigned()` / `api_annotator_load()`：使用 `bj_today_utc()` 过滤
- `api_history`：返回 `created_at` 字段改为北京时间字符串

### 附-2 分配批次号统一（v2.1→v3.0）

**方案**：
- `RawData` 模型新增 `dispatch_batch_no` 字段（v2.1）
- 分配时：提交后**统一生成批次号** `DISP-YYYYMMDD-NNN`，同时写入 `DispatchLog.batch_no` 和 `RawData.dispatch_batch_no`
- `annotation_list.html` 表格新增"批次号"列
- URL 参数 `?batch_no=DISP-YYYYMMDD-NNN` 直接打开对应批次任务

### 附-3 撤回与删除功能

**状态流转**：
- **未开始/进行中**：操作列显示"撤回"按钮（橙色），点击释放未标注任务，删除分配记录
- **已完成**：操作列显示"删除"按钮（灰色），点击删除记录但保留已标注数据

**API**：`POST /api/dispatch/revoke-batch`
- 参数：`batch_no`（单个）或 `batch_nos`（数组）
- 返回：`revoked_count`（释放数）、`completed_count`（完成保留数）、`total_count`

### 附-4 批量撤回

**前端**：
- 分配历史表格新增复选框列
- 表头全选 checkbox + 表尾选中计数 + "批量撤回"按钮
- 支持按行撤回（单条操作按钮）和批量撤回（表尾按钮）

### 附-5 标注任务页返回按钮

- 从调度中心"查看任务"进入时，页面左上角显示"← 返回调度中心"按钮
- 点击后通过 `parent.navigateTo()` 同步侧边栏高亮，返回调度中心
- 批次号标签（`📦 DISP-YYYYMMDD-NNN`）显示在标题旁

### 附-6 侧边栏联动

- `index.html` 新增 `window.navigateTo(page)` 全局函数
- 更新 iframe `src` + 同步侧边栏高亮状态
- `annotation_list.html` 调用 `parent.navigateTo('/dispatch_center.html')` 返回

### 附-7 统计摘要改为准确率进度条

**原版**：6 张 KPI 数字卡片（标注总数/待标注/已标注/正确/错误/忽略）

**v3.0**：3 个准确率进度条

| 指标 | 计算公式 | 颜色 |
|------|---------|------|
| 整体准确率 | correct / (correct + error) | 蓝色 primary |
| 合规数据准确率 | AI合规中 correct / (correct + error) | 绿色 success |
| 违规数据准确率 | AI违规中 error / (correct + error) | 红色 danger |

**辅助信息**：辅助文字显示原始计数（总数/待标/已标/正确/错误/忽略）

### 附-8 分组视图合规/违规数

- Group 视图统计栏：`分组数 / 合规数（绿）/ 违规数（红）/ 已标注数`
- Group 卡片内部：`✅ 合规 N / ❌ 违规 N / ⏳ 待标 N / 共 N 条`
...
## 11. 机审中心与调度中心全链路联动 v3.1

**实施日期**：2026-05-26

### 11.1 问题背景

机审中心当前"生成标注任务"后，批次状态与实际 RawData 记录之间缺乏细粒度的状态追踪：
- 管理员无法在明细页直接看到每条数据的分配/标注状态
- 无法快速从机审中心跳转调度中心追踪分配进度
- "重新生成"功能未区分已领取任务和未领取任务，存在误撤回风险

### 11.2 功能总览

| 模块 | 功能点 | 优先级 |
|------|--------|--------|
| 机审中心 | "生成标注任务"按钮 → 双按钮（task_generated=true 时） | P0 |
| 机审中心 | 重新生成任务（智能撤回 + 重新生成） | P0 |
| 机审中心 | 查看生成任务（跳转到调度中心） | P0 |
| 明细页 | 新增"任务分配状态"筛选（未分配/已分配/已标注） | P0 |
| 明细页 | 新增"标注员"和"标注结果"列 | P0 |
| 明细页 | 标注状态标签 hover 显示分配批次号 | P1 |
| 调度中心 | 支持 URL 参数 `filter_batch` 自动筛选指定批次 | P0 |
| 后端 | 新增 `POST /api/dispatch/regenerate-task-pool` | P0 |
| 后端 | 分配成功后同步 RawData.task_status | P0 |

### 11.3 数据状态定义

| task_status | 条件 | 含义 |
|-------------|------|------|
| `unassigned` | `annotator` 为空/null 且 `check_result` 为空/null | 未分配，仍在任务池 |
| `assigned` | `annotator` 非空 且 `check_result` 为空/null | 已分配给标注员，待标注 |
| `annotated` | `check_result` 非空（correct/error/ignore） | 已完成标注 |

> 历史数据（无 `task_status` 字段）：后端实时计算派生值，前端不依赖字段存在。
...
批次已互检
    ↓ 点击"生成标注任务"
批次 task_generated=true
    ↓
机审中心"更多"菜单 → "查看生成任务"
    → 新标签打开 dispatch_center.html?filter_batch=BAT-XXX
    → 调度中心自动显示该批次所有任务
    ↓
机审中心"更多"菜单 → "重新生成任务"
    → 后端自动撤回未被领取的任务
...
明细页 → 筛选"已分配"
    → 显示标注员、标注结果
    → hover 标注状态标签 → 显示分配批次号
```

### 11.5 UI 变更

#### 11.5.1 机审中心列表页 — 操作按钮逻辑
...
|--------|---------|
| 标注员 | 有值显示姓名，无值显示"——" |
| 标注结果 | unassigned→绿色"未分配"；assigned→蓝色"待标注"+hover批次号；annotated→正确/错误/忽略 标签 |

#### 11.5.4 调度中心 — filter_batch 参数

- `dispatch_center.html?filter_batch=BAT-20260525101908`
- 初始化时自动在任务池视图中筛选对应批次
...
| # | 验收条件 | 验证方式 |
|---|----------|----------|
| V1 | 分配成功后，RawData.task_status='assigned' | DB 查询 |
| V2 | 标注提交后，RawData.task_status='annotated' | DB 查询 |
| V3 | 明细页筛选"已分配" → 仅显示 assigned 记录 | UI 操作 |
| V4 | 明细页"标注结果"列：待标注标签 hover 显示分配批次号 | UI hover |
| V5 | 点击"查看生成任务" → 新标签打开调度中心，任务池自动显示对应批次 | UI 操作 |
| V6 | 对部分任务被领取的批次点击"重新生成" → 未领取任务被撤回，已领取任务不受影响 | UI 操作 + DB |
| V7 | 重新生成后，任务池数量 = 新生成数量 | 任务池数字验证 |

---

## 12. 附录：代码审查发现的 PRD 偏差清单
